...
<div style="width: 100%;float:left;margin-right: 25px;overflow: auto;">
 ...
 <table id="pricetable" class="draggable widefat" border="0" width="100%" cellspacing="0" cellpadding="0" >
   ...
   <tr class="nodrag nodrop">
       <td class="frow"><strong><span>Features|Packages</span></strong></td>  
    ...
    foreach($package_name as $index => $value){ 
        ...
        echo  '<td class="'.$index.' dragPricingCol" title="You can rearrange Packages by Drag and Drop"><img src="'. plugins_url().'/pricing-table/images/delete.png" class="deletecol" rel="'.$index.'" title="Delete this column" />
        <strong><span id="sp'.$index.'">
        '.$value.'
        </span>
        </strong>
        <input type="hidden" name="package_name['.$index.']" id="val_'.$index.'" value="'.$value.'" />
        <img rel="'.$value.'"  id="f'.$imgc.'" class="featured-package" title="Click here to feature this Package" src="'. plugins_url().'/pricing-table/images/'.$fimg.'">
      <img rel="sp'.$index.'"  id="e'.$index.'" class="featured-package-edit" title="Click here to edit Package name" src="'. plugins_url().'/pricing-table/images/edit.png"  /> 
      </td>';
    }
    }
?>
        ...
        foreach($package_name as $index => $value){
            ...
            if($t==1){
                ...
                if($value1 != "Price" && $value1!="Detail" && $value1!="ButtonURL" && $value1!="ButtonText" ){
                    ...
                    echo '<img class="rdndHandler" title="Use this handle to rearrange this row" src="'.plugins_url().'/pricing-table/images/updown.png" />';
...
<td valign="top" class="apcl"><a style="float: right;" href="#"  class="add-package" id="addcolumn">Add Package</a></td>
</tr>
...
<table class="widefat">
    <thead>
        <tr><th colspan="2"><strong>PayPal Settings</strong> ( Only "rock" template supports PayPal )</th></tr>
    </thead>
    <tr>
        <td><strong>Return URL</strong></td>
        <td><input size="80" type="text" placeholder="Available in pro only!" disabled="disabled" name="__wppt_returnurl" id="__wppt_returnurl" value="<?php echo get_post_meta($post->ID, '__wppt_returnurl',true);  ?>"></td>
    </tr>
    <tr>
        <td><strong>Cancel URL</strong></td>
        <td><input size="80" type="text" placeholder="Available in pro only!" disabled="disabled" name="__wppt_cancelurl" id="__wppt_cancelurl" value="<?php echo get_post_meta($post->ID, '__wppt_cancelurl',true);  ?>"></td>
    </tr>
    <tr>
        <td><strong>Currency Code</strong></td>
        <td><input size="80" type="text" placeholder="Available in pro only!" disabled="disabled" name="__wppt_currency_code" id="__wppt_currency_code" value="<?php echo get_post_meta($post->ID, '__wppt_currency_code',true);  ?>"></td>
    </tr>
    <tr>
        <td><strong>PayPal Account Email</strong></td>
        <td><input size="80" type="text" placeholder="Available in pro only!" disabled="disabled" name="__wppt_business" id="__wppt_business" value="<?php echo get_post_meta($post->ID, '__wppt_business',true);  ?>"></td>
    </tr>
</table><br /><br />
